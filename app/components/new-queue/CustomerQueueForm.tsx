...
export default function CustomerQueueForm({ user_id }: Props) {
	...
	async function getAvalibleSlotsTime() {
		setTimeSlots([]);
		try {
			const today = moment(form.getValues().booking_date).format(
				"YYYY-MM-DD"
			);
			...
			const start = moment(form.getValues().booking_date)
				.startOf("day")
				.toDate();
			const end = moment(form.getValues().booking_date)
				.endOf("day")
				.toDate();
	...
	function countOverlapQueue(timeSlot: TimeSlot): number {
		const overlappingQueues = queues.filter((q) => {
			...
			return (
				qStart.isBefore(timeSlot.endTime) &&
				qEnd.isAfter(timeSlot.startTime)
			);
		});
							...
							<>
								{timeSlots.length > 0 && (
									<FormItem>
										...
										<FormLabel>
											เลือกเวลาที่ต้องการ (1 รอบ/30 นาที)
										</FormLabel>
										<RadioGroup
